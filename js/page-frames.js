...
// Detect url for frames, create frames
    ...
    if (l > 0) {
        if (_contents.find(o => o.url === p[0])) {
            page = _contents.find(o => o.url === p[0]);
...
function makeIframes(uri) {
    ...
    rus.addEventListener ("load", function () {
        eng.addEventListener ("load", function () {
            ...
            if (rusLinks && rusLinks.length > 0 && engLinks && engLinks.length > 0 && rusLinks.length === engLinks.length) {
                for (let i = 0; i < rusLinks.length; i++) {
                    let idr = rusLinks[i].getAttribute("href").substr(1);
                    let ide = engLinks[i].getAttribute("href").substr(1);
                    let rusTo = rus.contentWindow.document.getElementById(idr);
                    let engTo = eng.contentWindow.document.getElementById(ide);
                    if (rusTo && engTo) {
    ...
    rus.contentWindow.onscroll = function () {
        if (engScroll === true) {
            engScroll = false;
            return;
        }
    ...
    eng.contentWindow.onscroll = function () {
        if (rusScroll === true) {
            rusScroll = false;
            return;
        }
